...
class PrecisionSlider(QWidget):
    def __init__(self, min=0.0, max=1.0, max_magnification=50, magnifier_height=200):
        ...
        self.square_coef = (
                           1 - 1 / self.max_magnification) / self.magnifier_height ** 1.5
    ...
    def val_to_vis(self, val):
        ...
        w = size.width()
        p = self.padding_side
        return int(round(p + (w-2*p)*(val-self.min_val)/(self.max_val-self.min_val)))

    def vis_to_val_relative(self, vis_rel):
        size = self.size()
        w = size.width()
        p = self.padding_side
        return (self.max_val - self.min_val) * vis_rel / (
        w - 2 * p)
...
class PrecisionSingleSlider(PrecisionSlider):
    ...
    def mousePressEvent(self, ev):
        ...
        if self.mouse_status > 0:
            ...
            global_xy = self.mapToGlobal(QPoint(self.mouse_start_x,
                                                self.mouse_start_y))

            self.popup.setGeometry(global_xy.x()-self.magnifier_height//2,
                                   global_xy.y(),
...
class RangeSliderWidget(PrecisionSlider):
    ...
    def drawWidget(self, qp):
        ...
        qp.setRenderHint(QPainter.Antialiasing)
        self.l_triangle, self.r_triangle = (self._equilateral_triangle_points(
            (val, pt + self.barwidth)) for val in [lv, rv])

        for triangle, label in zip([self.l_triangle, self.r_triangle],
                                   [1, 2]):
            if self.mouse_status == label:
                qp.setBrush(self.highlight_color)
            else:
    ...
    def mousePressEvent(self, ev):
        ...
        if (self.l_triangle[0][0] < self.mouse_start_x < self.r_triangle[0][0]) and \
               (self.l_triangle[0][1]-self.barwidth-2 < ev.y() < self.l_triangle[0][1]+2):
            self.mouse_status = 3

        if self.mouse_status > 0:
            global_xy = self.mapToGlobal(
                QPoint(self.mouse_start_x, self.mouse_start_y))
            self.popup.show()
            self.popup.setGeometry(global_xy.x()-self.magnifier_height//2,
                                   global_xy.y(), self.magnifier_height,
                                   self.magnifier_height)
